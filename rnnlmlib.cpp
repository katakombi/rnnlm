...
int CRnnLM::addWordToVocab(char *word)
{
    ...
    if (vocab_size + 2 >= vocab_max_size) { //reallocate memory if needed
        vocab_max_size += 100;
        vocab =
            (struct vocab_word *)realloc(vocab,
                                         vocab_max_size *
...
void CRnnLM::saveWeights()      //saves current weights and unit activations
{
    ...
    if (layerc_size > 0) {
        for (b = 0; b < layerc_size; b++)
            for (a = 0; a < layer1_size; a++) {
                syn1b[a + b * layer1_size].weight =
                    syn1[a + b * layer1_size].weight;
            }

        for (b = 0; b < layer2_size; b++)
            for (a = 0; a < layerc_size; a++) {
                syncb[a + b * layerc_size].weight =
...
void CRnnLM::initNet()
{
    ...
    syn0 =
        (struct synapse *)calloc(layer0_size * layer1_size,
    ...
    if (layerc_size == 0)
        ...
        syn1 =
            (struct synapse *)calloc(layer1_size * layerc_size,
                                     sizeof(struct synapse));
        sync =
            (struct synapse *)calloc(layerc_size * layer2_size,
                                     sizeof(struct synapse));
    }
    ...
    if ((syn_d == NULL) && (filetype != COMPRESSED)) {
        fprintf(stderr,
                "Memory allocation for direct connections failed (requested %lld bytes)\n",
    ...
    if (bptt > 0) {
        ...
        //
        bptt_hidden =
            (neuron *) calloc((bptt + bptt_block + 1) * layer1_size,
                              sizeof(neuron));
...
void CRnnLM::restoreNet()       //will read whole network structure
{
    ...
    goToDelimiter(':', fi);
    fscanf(fi, "%d", &ver);
    if ((ver == 4) && (version == 5))   /* we will solve this later.. */
        ;
    else if (ver != version) {
    ...
    if (filetype == COMPRESSED) {
        ...
        syn_cd =
            (uint8_t *) calloc((long long)((direct_size * ncluster) / 8 + 2),
                               sizeof(uint8_t));
...
void CRnnLM::kmean()
{
    ...
    do {
        ...
        for (i = 0; i < NUMCENTS; i++)
            centroid[i] = counts[i] ? tempcent[i] / counts[i] : centroid[i];

        // sort centroid values
        qsort(centroid, NUMCENTS, sizeof(direct_t), compare);
        for (i = 0; i < NUMCENTS; i++)
            fprintf(stderr, "%2.3g ", centroid[i]);
...
void CRnnLM::computeNet(int last_word, int word)
{
    ...
    if (layerc_size > 0) {
        matrixXvector(neuc, neu1, syn1, layer1_size, 0, layerc_size, 0,
                      layer1_size, 0);
    ...
    if (layerc_size > 0) {
        matrixXvector(neu2, neuc, sync, layerc_size, vocab_size, layer2_size, 0,
                      layerc_size, 0);
    } else {
        matrixXvector(neu2, neu1, syn1, layer1_size, vocab_size, layer2_size, 0,
                      layer1_size, 0);
    }
    ...
    if (word != -1) {
        ...
        if (layerc_size > 0) {
            matrixXvector(neu2, neuc, sync, layerc_size,
                          ...
                          class_words[vocab[word].class_index][0] +
                          class_cn[vocab[word].class_index], 0, layerc_size, 0);
        } else {
            matrixXvector(neu2, neu1, syn1, layer1_size,
                          class_words[vocab[word].class_index][0],
                          class_words[vocab[word].class_index][0] +
                          class_cn[vocab[word].class_index], 0, layer1_size, 0);
        }
    }
...
void CRnnLM::learnNet(int last_word, int word)
{
    ...
    if (direct_size > 0) {      //learn direct connections between words and classes
        ...
        for (a = 0; a < direct_order; a++) {
            ...
            for (b = 1; b <= a; b++)
                hash[a] +=
                    PRIMES[(a * PRIMES[b] +
                            b) % PRIMES_SIZE] * (unsigned long long)(history[b -
                                                                             1]
                                                                     + 1);
    ...
    if (layerc_size > 0) {
        matrixXvector(neuc, neu2, sync, layerc_size,
                      class_words[vocab[word].class_index][0],
                      class_words[vocab[word].class_index][0] +
                      class_cn[vocab[word].class_index], 0, layerc_size, 1);
        ...
        matrixXvector(neu1, neu2, syn1, layer1_size,
                      class_words[vocab[word].class_index][0],
                      class_words[vocab[word].class_index][0] +
                      class_cn[vocab[word].class_index], 0, layer1_size, 1);
        ...
        if (((counter % bptt_block) == 0) || (independent && (word == 0))) {
            for (step = 0; step < bptt + bptt_block - 2; step++) {
                ...
                for (a = 0; a < layer1_size; a++) { //propagate error from time T-n to T-n-1
                    neu1[a].er =
                        neu0[a + layer0_size - layer1_size].er +
                        bptt_hidden[(step + 1) * layer1_size + a].er;
                }
            ...
            for (b = 0; b < layer1_size; b++) { //copy temporary syn0
                if ((counter % 10) == 0) {
                    for (a = layer0_size - layer1_size; a < layer0_size; a++) {
                        syn0[a + b * layer0_size].weight +=
                            bptt_syn0[a + b * layer0_size].weight - syn0[a +
                                                                         b *
                                                                         layer0_size].
                            weight * beta2;
                    ...
                    for (step = 0; step < bptt + bptt_block - 2; step++)
                        if (bptt_history[step] != -1) {
                            syn0[bptt_history[step] + b * layer0_size].weight +=
                                bptt_syn0[bptt_history[step] +
                                          b * layer0_size].weight -
                                syn0[bptt_history[step] +
                                     b * layer0_size].weight * beta2;
                            bptt_syn0[bptt_history[step] +
                                      b * layer0_size].weight = 0;
                        }
                } else {
                    for (step = 0; step < bptt + bptt_block - 2; step++)
                        if (bptt_history[step] != -1) {
...
void CRnnLM::trainNet()
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            if ((counter % 10000) == 0)
                if ((debug_mode > 1)) {
                    ...
                    if (train_words > 0)
                        fprintf(stderr,
                                "%cIter: %3d\tAlpha: %f\t   TRAIN entropy: %.4f    Progress: %.2f%%   Words/sec: %.1f ",
                                13, iter, alpha, -logp / log10(2) / counter,
                                counter / (real) train_words * 100,
                                counter / ((double)(now - start) / 1000000.0));
                    else
                        fprintf(stderr,
                                "%cIter: %3d\tAlpha: %f\t   TRAIN entropy: %.4f    Progress: %dK",
                                13, iter, alpha, -logp / log10(2) / counter,
                                counter / 1000);
                    fflush(stdout);
                }
            ...
            if (word != -1)
                logp +=
                    log10(neu2[vocab[word].class_index + vocab_size].ac *
                          neu2[word].ac);
            ...
            if (bptt > 0) {     //shift memory needed for bptt to next time step
                ...
                for (a = bptt + bptt_block - 1; a > 0; a--)
                    for (b = 0; b < layer1_size; b++) {
                        bptt_hidden[a * layer1_size + b].ac =
                            bptt_hidden[(a - 1) * layer1_size + b].ac;
                        bptt_hidden[a * layer1_size + b].er =
                            bptt_hidden[(a - 1) * layer1_size + b].er;
                    }
            }
            //
        ...
        now = clock();
        fprintf(stderr,
                "%cIter: %3d\tAlpha: %f\t   TRAIN entropy: %.4f    Words/sec: %.1f   ",
                13, iter, alpha, -logp / log10(2) / counter,
                counter / ((double)(now - start) / 1000000.0));
    ...
    while (1) {
        ...
        if ((word != -1) || (prob_other > 0)) {
            if (word == -1) {
                ...
                log_combine +=
                    log10(neu2[vocab[word].class_index + vocab_size].ac *
                          neu2[word].ac * lambda + prob_other * (1 - lambda));
            }
...
void CRnnLM::testNet()
{
    ...
    while (1) {
        ...
        if (debug_mode > 1) {
            if (use_lmprob) {
                if (word != -1)
                    fprintf(flog, "%d\t%.10f\t%.10f\t%s", word,
                            neu2[vocab[word].class_index +
                                 vocab_size].ac * neu2[word].ac, prob_other,
                            vocab[word].word);
                else
                    fprintf(flog, "-1\t0\t\t0\t\tOOV");
            } else {
                if (word != -1)
                    fprintf(flog, "%d\t%.10f\t%s", word,
                            neu2[vocab[word].class_index +
                                 vocab_size].ac * neu2[word].ac,
                            vocab[word].word);
                else
                    fprintf(flog, "-1\t0\t\tOOV");
            }
...
void CRnnLM::testNbest()
{
    ...
    if (use_lmprob) {
        ...
        lambda = 1;             //!!! for simpler implementation later
    ...
    while (1) {
        if (last_word == 0) {
            ...
            if (nbest_cn == 1)
                saveContext2(); //save context after processing first sentence in nbest
        ...
        if (word != -1) {
            ...
            log_combine +=
                log10(neu2[word].ac * lambda + prob_other * (1 - lambda));
...
void CRnnLM::testGen()
{
    ...
    while (wordcn < gen) {
        ...
        for (c = 0; c < class_cn[cla]; c++)
            neu2[class_words[cla][c]].ac = 0;
        matrixXvector(neu2, neu1, syn1, layer1_size, class_words[cla][0],
                      class_words[cla][0] + class_cn[cla], 0, layer1_size, 0);
                ...
                for (a = 0; a < direct_order; a++) {
                    ...
                    if (a > 0)
                        if (history[a - 1] == -1)
                            break;
                    hash[a] =
                        PRIMES[0] * PRIMES[1] * (unsigned long long)(cla + 1);
